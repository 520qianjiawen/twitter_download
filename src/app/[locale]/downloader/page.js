...
export default function Downloader({ params: { locale } }) {
    ...
    const downloadAllMedia = async () => {
        const zip = new JSZip();
        // 先创建一个文件夹
        // Use a more generic folder name that reflects the new branding.
        const folder = zip.folder('medias_from_twitter_download');
        const tempTweets = [...tweets];
        for(let i = 0; i < tempTweets.length; i++){
            const tweet = tempTweets[i];
        ...
        // 添加.URL文件和.webloc文件
        // Point users back to the new homepage when they open the .URL or .webloc file.
        folder.file('download_more.URL', 'https://x.neutemu.com/');
        folder.file('download_more.webloc', '<dict><key>URL</key><string>https://x.neutemu.com/</string></dict>');
        ...
        a.href = url;
        a.download = 'medias_from_twitter_download.zip';
